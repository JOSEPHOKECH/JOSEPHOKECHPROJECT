...
session_start();
if(isset($_GET['id'])){
$qry = $conn->query("SELECT t.*,concat(p.lastname,', ',p.firstname,' ',p.middlename) as name, concat(p.address,', ',p.street,', ',p.phone,', ',p.city,', ',p.state,', ',p.zip_code) as caddress,e.name as ename,p.tracking_id FROM person_tracks t inner join persons p on p.id = t.person_id inner join establishments e on e.id = t.establishment_id  where t.id= ".$_GET['id']);
	...
	<form action="" id="manage-records">
		...
		<div id="details" <?php echo isset($id) ? "style='display:block'" : 'style="display:none"' ?>>
			...
			<div class="form-group">
				...
				<input type="text" class="form-control" name="store"  value="<?php echo isset($store) ? $store :'' ?>" required>
			</div>
